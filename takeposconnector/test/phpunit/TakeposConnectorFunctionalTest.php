...
/**
 * \file    test/phpunit/TakeposConnectorFunctionalTest.php
...
/**
 * Class TakeposConnectorFunctionalTest
 *
 ...
 *
 * @package Testtakeposconnector
 */
class TakeposConnectorFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modTakeposConnector')->click();
		} else {
			// Disable the module
			$this->byHref('modTakeposConnector')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/takeposconnector/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('takeposconnector/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/takeposconnector/admin/about.php');
		$this->authenticate();
		return $this->assertContains('takeposconnector/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('takeposconnectorwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modTakeposConnector_TakeposConnectorTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modTakeposConnector_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
